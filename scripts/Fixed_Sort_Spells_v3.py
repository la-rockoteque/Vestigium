...
def read_spells(file_path):
    ...
    def extract_level(spell):
        match = re.search(r'\b(Cantrip|\d+)[stndrdth]*[-\s]?level\b', spell, re.IGNORECASE)
        return -1 if "cantrip" in match.group(1).lower() else int(match.group(1)) if match else 100
    ...
    spell_entries.sort(key=lambda spell: (extract_level(spell), extract_spell_name(spell)))
