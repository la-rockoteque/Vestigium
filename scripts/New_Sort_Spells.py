...
def read_spells(file_path):
    ...
    def extract_level(spell_lines):
        ...
        match = re.search(r'\b(Cantrip|\d+)[stndrdth]*[-\s]?level\b', full_text, re.IGNORECASE)
        return -1 if match and "cantrip" in match.group(1).lower() else int(match.group(1)) if match else 100

    # Sort spells by level first, then alphabetically (handle None safely)
    spells.sort(key=lambda spell: (extract_level(spell[1]), spell[0].lower() if spell[0] else "zzz"))
