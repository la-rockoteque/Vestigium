...
def extract_spells(file_path, output_file):
    ...
    for i in range(0, len(spell_entries), 2):
        ...
        acquisition_note_match = re.search(
            r"\*\*Acquisition Note:\*\* (.*?)\n", spell_details, re.DOTALL
        )
        arcane_strain_match = re.search(
            r"\*\*Arcane Strain:\*\* (.*?)\n", spell_details, re.DOTALL
        )
        ...
        scaling_match = re.search(
            r"(?:\*\*Scaling \(Upcast Effect\):\*\*\n([\s\S]*?))?(?:\nAt Higher Levels\..*?)?(?=\n(?:\*\*Class:\*\*|\*\*Arcane Strain:\*\*|\*\*Acquisition Note:\*\*|\*\*Permit & Cost:\*\*))",
            spell_details,
        )
        scaling = (
            scaling_match.group(1).strip()
            if scaling_match and scaling_match.group(1)
            else ""
        )
        ...
        level = (
            ...
            if level_match
            else (cantrip_match.group(1).strip() if cantrip_match else "")
        )
        school = (
            level_match.group(3).strip()
            if level_match
            else (cantrip_match.group(2).strip() if cantrip_match else "")
        )
        ...
        acquisition_note = (
            acquisition_note_match.group(1).strip() if acquisition_note_match else ""
        )
        arcane_strain = (
            arcane_strain_match.group(1).strip() if arcane_strain_match else ""
        )
        ...
        component_description = (
            f"({component_parts[1].strip()}" if len(component_parts) > 1 else ""
        )
