...
def get_features_for_class(class_name):
  def get_feature_label(row):
    name = row.get("Name")
    level = int(row.get("Level"))

    return f"{name}|{class_name}|VestigiumGuidetoConcordCity|{level}"

  return [
    *[
      get_feature_label(entry_row)
      for index, entry_row in df_class_features.iterrows()
      if pd.notnull(entry_row.get("Class")) and pd.notnull(entry_row.get("Name")) and pd.isnull(entry_row.get("Parent")) and str(entry_row.get("Class")) == class_name and pd.isnull(entry_row.get("Subclass"))
    ]
  ]
...
def to_table(class_name: str):
  ...
  def process(row):
    proficiency = int(row.get("Proficiency Bonus")) if not pd.isnull(row.get("Proficiency Bonus")) else ""
    knownSpells = int(row.get("Spells Known")) if not pd.isnull(row.get("Spells Known")) else ""
    maxSpellLevel = int(row.get("Max Spell Level")) if not pd.isnull(row.get("Max Spell Level")) else ""
    Points = int(row.get("Points")) if not pd.isnull(row.get("Points")) else ""
    spellSlots = int(row.get("Total spell slots")) if not pd.isnull(row.get("Total spell slots")) else ""
    feature1 = int(row.get("Feature 1")) if not pd.isnull(row.get("Feature 1")) else ""
    return [
...
def to_spell_progression_table(class_name: str):
  labels = [
    f"{{@filter {level}|spells|level={level[0]}|class={class_name}}}" for level in ["1st","2nd","3rd","4th","5th","6th","7th","8th","9th"]
  ]

  tablish = [
    [row.get("1"), row.get("2"), row.get("3"), row.get("4"), row.get("5"), row.get("6"), row.get("7"), row.get("8"), row.get("9")]
    for index, row in df_class_tables.iterrows()
    if pd.notnull(row.get("Class")) and str(row.get("Class")).strip() == class_name
  ]
...
def get_features_for_class(class_name):
  ...
  return [
    *[
      get_feature_label(entry_row)
      for index, entry_row in df_class_features.iterrows()
      if pd.notnull(entry_row.get("Class")) and pd.notnull(entry_row.get("Name")) and pd.isnull(entry_row.get("Parent")) and str(entry_row.get("Class")) == class_name
    ]
  ]
...
def row_to_class(row):
  ...
  default_equipment = [
    ", ".join([f"{{@item {item}|{source}}}" for item in row.get("A", "").split(", ")]),
    ", ".join([f"{{@item {item}|{source}}}" for item in row.get("B", "").split(", ")]),
    ", ".join([f"{{@item {item}|{source}}}" for item in row.get("Common", "").split(", ")])
  ]
  ...
  classes = {
    ...
    "startingProficiencies": {
      ...
      "weapons": [
        *[
          ...
          for weapon in weapons.split(", ") if weapon not in ["simple", "martial"]
        ],
        "simple",
      ],
      "skills": [
    ...
    **({"spellcastingAbility": row.get("Spellcasting Ability")[:3].lower()} if not pd.isnull(row.get("Spellcasting Ability")) else {}),
    **({"casterProgression": row.get("Caster Progression")} if not pd.isnull(row.get("Caster Progression")) else {}),
    **({"preparedSpells": f"<$level$> + <${row.get("Spellcasting Ability")[:3].lower()}_mod$>"} if pd.isnull(row.get("Prepares Spells")) == "TRUE" else {}),
    **({"cantripProgression": cantrip_progression} if not pd.isnull(row.get("spellcastingAbility")) else {}),
    "classTableGroups": [
      to_table(class_name),
      *([to_spell_progression_table(class_name)] if not pd.isnull(row.get("spellcastingAbility")) else []),
    ],
    **({"subclassTitle": row.get("Subclass Title")} if not pd.isnull(row.get("Subclass Title")) else {}),
    "classFeatures": get_features_for_class(class_name),
    **({"classSpells": class_spells} if not pd.isnull(row.get("spellcastingAbility")) else {}),
  }
  return classes
