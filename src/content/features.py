...
def row_to_feature_entries(row):
  ...
  sub_entries = [
    ...
    if pd.notnull(entry_row.get("Class")) and pd.notnull(entry_row.get("Parent")) and str(entry_row.get("Parent")) == name and str(entry_row.get("Class")) == classes and str(entry_row.get("Parent")) != str(entry_row.get("Name"))
  ]

  entries = [
    *([entry] if not pd.isnull(row.get("Entry")) and not any(x["type"] == "abilityDc" for x in sub_entries) else []),
    *(([{ "type": "entries", "name": "Spellcasting Ability", "entries": [entry] + sub_entries}] if any(x["type"] == "abilityDc" for x in sub_entries) else [sub_entries]) if len(sub_entries) > 0 else [])
  ]
  ...
  return {
    ...
    **({"attributes": attribute.split(", ")} if not pd.isnull(row.get("Attributes")) else {}),
  }
...
def row_to_features(row):
  ...
  type = row.get("Type")
  entry = row.get("Entry")
  sub_entries = [      row_to_feature_entries(entry_row)
                       for index, entry_row in df_class_features.iterrows()
  ...
  entries = [
    ...
    *(([{ "type": "entries", "name": "Spellcasting Ability", "entries": [entry] + sub_entries}] if len(sub_entries) > 0 else [sub_entries]) if len(sub_entries) > 0 else [])
  ]
  return {
...
features_list = [
  ...
  if pd.notnull(row.get("Name")) and pd.isnull(row.get("Parent")) and row.get("Source") == source and pd.isnull(row.get("Subclass"))
]

sub_class_features_list = [
  row_to_subclass_features(row)
  for index, row in df_class_features.iterrows()
  if pd.notnull(row.get("Name")) and pd.isnull(row.get("Parent")) and row.get("Source") == source and pd.notnull(row.get("Subclass"))
]