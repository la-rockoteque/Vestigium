...
def row_to_monster(row):
  languages = row.get("Languages").lower().split(", ") if pd.notnull(row.get("Languages")) else []
  return {
    "source": json_source,
    "name": row.get("Name"),
    "size": [
      row.get("Size")[:1].upper()
    ],
    "type": row.get("Classic 5e Type").lower(),
    "alignment": [
      row.get("Alignment")[:1].upper()
    ],
    "ac": [
      row.get("Armor Class")
    ],
    ...
    "speed": {
      **({"walk": row.get("Speed (Walking)")} if pd.notnull(row.get("Speed (Walking)")) else {}),
      **({"fly": row.get("Speed (Flying)")} if pd.notnull(row.get("Speed (Flying)")) else {}),
      **({"swim": row.get("Speed (Swimming)")} if pd.notnull(row.get("Speed (Swimming)")) else {}),
      **({"burrow": row.get("Speed (Burrowing)")} if pd.notnull(row.get("Speed (Burrowing)")) else {})
    },
    "str": row.get("STR"),
    "dex": row.get("DEX"),
    "con": row.get("CON"),
    ...
    "cha": row.get("CHA"),
    **({"immune":
          row.get("Condition Immunities").lower().split(", ")
        } if pd.notnull(row.get("Condition Immunities")) else {}),
    # **({"languages":
